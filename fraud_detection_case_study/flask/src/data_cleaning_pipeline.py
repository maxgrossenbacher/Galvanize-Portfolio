...
class Data_Cleaning(object):
    ...
    def __init__(self, filepath):
        ...
        try:
            ...
            # self.df = pd.read_json(json.loads(filepath))
            self.df = pd.DataFrame.from_dict(filepath, orient='index').transpose()
        finally:
            print('you suck')

    def _cleaning_data(self):
        # Manually change series to dataframe
    ...
    def _removing_data(self):
        ...
        self.df.drop(['object_id', 'currency', 'has_header', 'venue_longitude', 'delivery_method', 'payout_type', 'listed', 'venue_longitude', 'venue_latitude',
    ...
    def run_pipeline(self):
        ...
        if y_test_probs[:, 1] < .3:
            return(('Low Risk'), (y_test_probs[:, 1]))
        elif y_test_probs[:, 1] < .6:
            return(('Medium Risk'), (y_test_probs[:, 1]))
        else:
            return(('High Risk'), (y_test_probs[:, 1]))


# if __name__ == '__main__':
    # dc = Data_Cleaning('../data/streaming_data.json')
    # dc.run_pipeline()
