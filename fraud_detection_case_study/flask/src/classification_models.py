...
if __name__ == '__main__':
    X_train, X_test, y_train, y_test, cols = data_prep.data_processing('../data/2000sample_alltextdatesdropped.csv')
    # print('\n --> Decision Tree------------------')
    ...
    #                                     warm_start=False, \
    #                                     presort='auto')
    # gb_model, predictions, probabilities = trees_model(rf,X_train, X_test, y_train, y_test)
    ...
    print('\n --> Random Forest------------------')
    rf = RandomForestClassifier(criterion= 'gini', max_depth= 15, max_features= 10, min_impurity_decrease= 0.001, min_samples_split= 0.05, n_estimators= 1000)
    rf_model, predictions, probabilities = trees_model(rf,X_train, X_test, y_train, y_test)
    print('------------scoring model RF------------------')
    print('recall score: {}'.format(recall_score(y_test, predictions)))
    print('accuracy: {}'.format(rf_model.score(X_test, y_test)))
